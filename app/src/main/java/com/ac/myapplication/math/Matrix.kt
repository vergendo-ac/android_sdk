...
data class Mat2(
    ...
    operator fun plus(v: Float) =
        Mat2(x + v, y + v)

    operator fun minus(v: Float) =
        Mat2(x - v, y - v)
...
fun transpose(m: Mat2) =
    Mat2(
        Float2(m.x.x, m.y.x),
        Float2(m.x.y, m.y.y)
    )

fun transpose(m: Mat3) =
    Mat3(
...
fun lookAt(
    eye: Float3, target: Float3, up: Float3 = Float3(
        z = 1.0f
    )
): Mat4 {
...
fun lookTowards(
    eye: Float3, forward: Float3, up: Float3 = Float3(
        z = 1.0f
    )
): Mat4 {
...
fun ortho(l: Float, r: Float, b: Float, t: Float, n: Float, f: Float) =
    Mat4(
